...
int Windows::run_command(const char *cmd, bool redirection_stdout, bool trace)
{
    ...
    {
        std::filesystem::path stdout_path = build_dir;
        if (trace)
...
int Windows::run_command_pacman(const char *cmd)
{
    curr_path = std::filesystem::current_path();
...
int Windows::run_command_pacman_check(const char *cmd, string full_path)
{
    curr_path = std::filesystem::current_path();
    cerr << "Running PACMAN command {" << cmd << "} in {"<< curr_path << "}" << endl;
    int return_code = 1;
...
list<vector<string>> Windows::name_variants(string name)
{
    ...
    for (string i : name_var.front())
    {
        ...
        while (ind < second_type_name.size())
        {
         ...
         //если одна заглавная, то она просто станет строчной и будет повтор первого фильтра
        if (insert_tire)
...
int Windows::find_install_package_for_lib(std::filesystem::path pt, string dll, string a, string dlla, string lib, string stdout_path_str, bool *found_lib, bool library_not_found_anywhere)
{
    int return_code = 0;
    ...
    if (std::filesystem::exists(pt/dll))
    {
        if (found_lib)
            *found_lib = true;
    ...
    else
    {
        ...
        else
        {
            ...
            if (std::filesystem::exists(pt/dlla))
            {
                if (found_lib)
                    *found_lib = true;
            ...
            else
            {
                ...
                if (std::filesystem::exists(pt/lib))
                {
                    if (found_lib)
                        *found_lib = true;
...
void Windows::find_lib(vector<string> libs, vector<string> libsPath)
{
    string path;
    string check_l;
    int mypipe[2];
    ...
    for (auto l : libs)
    {
        ...
        if (type_libs[0] || type_libs[1] || type_libs[2])
        {
            for (auto p : libsPath) //ищем по всем путям указанным
            {
                ...
                {
                    if (std::filesystem::exists(path_to_lib/l))
                    {
...
void Windows::check_toolchain(regex mask1, regex mask2, std::filesystem::path pathToDir)
{
...
vector<string> Windows::linkDirectories(regex mask, std::filesystem::path pathToDir)
{
    string pathToFile;
...
vector<string> Windows::libpath(std::filesystem::path pathToJson)
{
...
void Windows::check_libs(vector<string> libs, std::filesystem::path buildDir)
{
    bool check = true;
    int size = libs.size();
...
vector<string> Windows::lib_list(std::filesystem::path pathToJson)
{
    vector<string> libs;
    ifstream i(pathToJson);
    json j;
...
void Windows::libs_info(regex mask, std::filesystem::path pathToDir, std::filesystem::path buildDir)
{
    //toolchain точно один
    regex mask1(".*(toolchains).*",regex_constants::icase);
    check_toolchain(mask1, mask, pathToDir);

    // Все каталоги, в которых стандартно ищутся библиотеки
    vector <string> linkDirs = linkDirectories(mask1, pathToDir);
    for (auto i: linkDirs)
        cout << i << " ";
    cout << endl;
...
int Windows::assembly_cmake()
{
    ...
    std::filesystem::path inc_dir {"C:\\Windows\\Temp\\archives\\oicb-master\\include"};
    regex mask1(".*(target).*",regex_constants::icase);

    std::filesystem::path reply_path = build_dir;
    reply_path /= ".cmake";
    reply_path /= "api";
    reply_path /= "v1";
    reply_path /= "reply";
...
void Windows::set_aname(string name)
{
    archive_name = name;
}

void Windows::set_bdir(std::filesystem::path p) {
    build_dir = p;
}

void Windows::set_udir(std::filesystem::path p) {
    unpack_dir = p;
}

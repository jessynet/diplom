    ...
    switch (pid) {
        ...
        default: //pid>0 родительский процесс
        {
            ...
            if (stdout_pipe) 
            {
                ...
                if(find_install_package_1_fb(stdout_pipe)!=0)
                    return_code = 1;   
    ...
    while((linelen = getline(&line, &linesize, f)) != -1)
    {
        ...
        {	
            ...
            if(name_package_for_lib != "")
            {
                ...
                found_package = true;
                if(run_command_1_fb({"pkg", "install", "-y", name_package_for_lib},true) == 0)
                {    
                    install = true;
...
void find_lib_fb(vector<string> libs, vector<string> libsPath)
{
    ...
    for(auto l : libs)
    {
        ...
        if(!found) //библиотека не нашлась ни по одному из путей
	    {
		    ...
		    for(auto i: path_copy)
			{
                libname_fb = i/nl;
                pipe(mypipe2);
                if(run_command_1_fb({"pkg", "provides", "^" + libname_fb}, false, mypipe2) == 0)
				    break;
            }
	    } 
    }
...
void check_toolchain_fb(regex mask1, regex mask2, fs::path pathToDir)
{
...
vector<string> linkDirectories_fb(regex mask, fs::path pathToDir)
{
    string pathToFile;
...
vector<string> libpath_fb(fs::path pathToJson)
{
...
void check_libs_fb(vector<string> libs, fs::path buildDir)
{
    bool check = true;
    int size = libs.size();
...
vector<string> lib_list_fb(fs::path pathToJson)
{
    vector<string> libs;
    ifstream i(pathToJson);
    json j;
...
void libs_fb(regex mask, fs::path pathToDir, fs::path buildDir)
{
    //toolchain точно один
    regex mask1(".*(toolchains).*",regex_constants::icase);

    check_toolchain_fb(mask1, mask, pathToDir);

    vector <string> linkDirs = linkDirectories_fb(mask1,pathToDir); //Все каталоги, в которых стандартно ищутся библиотеки
    ...
    for(const auto& entry : dirIterator)
    {
        if(!is_directory(entry))
        {
            if(regex_match(entry.path().c_str(),mask))
            {
                ...
                pathToFile = entry.path();
                libs = lib_list_fb(pathToFile);

                libPath = libpath_fb(pathToFile); //Тут пути с -L
                if(libs.size() != 0)
                {
                    for(auto j: libs)
                        cout << j << " ";
                    ...
                    if(libPath.size() != 0)
                    {
                        ...
                        check_libs_fb(libPath, buildDir);

                        find_lib_fb(libs, libPath);

                    }

                    else
                        find_lib_fb(libs, linkDirs);
                }
...
void find_depend_freebsd(fs::path path_to_reply, fs::path buildDir)
{

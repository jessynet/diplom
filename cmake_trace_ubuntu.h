...
vector<string> link_toolchain_dir_ub;
...
int find_install_package_for_lib_ub(fs::path path, string name_so, string name_a, bool *found_lib = nullptr, bool library_not_found_anywhere = false)
{
    ...
    if(library_not_found_anywhere)
    {
        string reg_path;
        reg_path = "^" + libname_ub;
        return_code_ub = run_command_1_ub({"apt-file", "-F", "search", reg_path}, false, mypipe);
        if(return_code_ub) 
        {
            pipe(mypipe);
...
list< vector<string> > name_variants_ub(string name)
{
        ...
        while(getline(in,line))
        {
            if(line[0] == '{')
            {
                ...
                else if(j["cmd"] == "find_package")
                {
                    ...
                    names = name_variants_ub(package_name);
                    int mypipe[2];
                    bool inst = false;
                    for(vector<string> elem : names)
                    {
                        ...
                        for(string i : elem)
                        {
                            ...
                            if(i.substr(0,3) != "lib") //не библиотека
                            {
                                ...
                                if(run_command_trace_ub({"apt","install", "-y", i},true) == 0)
                                {
                                ...
                                for(auto l : libs)
                                {
                                    for(auto path : standart_path)
                                        if(fs::exists(path/l))
                                        {
                                            ...
                                            reg_path = "^" + reg_path;
                                            if(run_command_1_ub({"apt-file", "-F", "search", reg_path}, false, mypipe) == 0)
                                            {
                                                lib_install = true;
                                                inst = true;
                                                break;
                    ...
                    for(auto i : names)
                    {
                        if(i != "")
                        {
                            ...
                            else
                            {
                                ...
                                vector <string> linkDirs = link_toolchain_dir_ub; //Все каталоги, в которых стандартно ищутся библиотеки
...
void ubuntu_trace(fs::path unpc_path, fs::path path_to_package, vector <string> link_dirs, string archive_name)
{
    chdir(unpc_path.c_str());
    path_to_package_lib_ub = path_to_package;
    link_toolchain_dir_ub = link_dirs;

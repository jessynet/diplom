...
void find_lib(vector<string> libs, vector<string> libsPath)
{
    ...
    //pipe(mypipe);
    //if(run_command_1({"apt-file", "search", libname}, false, mypipe) != 0)
    //    cout << "Не удалось найти пакет, который предоставляет библиотеку " << libname << endl;
...
int main()
{
    ...
    libs(mask1,"/tmp/archives/xz-5.6.0/build-xz-5.6.0/.cmake/api/v1/reply");

...
int run_command(const char* cmd, bool redirection_stdout = false, bool trace = false)
{
    ...
    {
        char path_file[] = "C:\\Windows\\Temp\\archives\\xz-master\\stdout.txt";
        string pathtmp = "C:\\Windows\\Temp\\archives\\" + archive_name + "\\trace-output";
        char *path_trace = new char[pathtmp.size() + 1];
        ...
        if(trace)
        {
            h = CreateFile(LPTSTR(path_trace), FILE_APPEND_DATA, FILE_SHARE_WRITE | FILE_SHARE_READ, &sa, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        ...
        //если функция выполняется неудачно, то возвращается нулевое значение
        {
            ...
            CloseHandle(h);
            //Удаляем файл
            if(!trace) DeleteFile(LPTSTR(path_trace));
            else DeleteFile(LPTSTR(path_file));
...
int run_command_pacman(const char* cmd)
{
    ...
    char path_file[] = "C:\\Windows\\Temp\\archives\\xz-master\\pacman_stdout.txt";
    
    //char path_file[] = "C:\\Windows\\Temp\\archives\\oicb-master\\pacman_stdout.txt";
    SECURITY_ATTRIBUTES sa;
    sa.nLength = sizeof(sa);
    sa.lpSecurityDescriptor = NULL;
    sa.bInheritHandle = TRUE;
    //перенаправление stdout в файл
    HANDLE h = CreateFile(LPTSTR(path_file), FILE_GENERIC_READ | FILE_APPEND_DATA, FILE_SHARE_WRITE | FILE_SHARE_READ, &sa, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if(h == INVALID_HANDLE_VALUE)
    {
        cout << "Ошибка дескриптора" << endl;
    }
                    ...
                    for(vector <string> elem : names)
                    {
                        for(string i : elem)
                        {
                            if(i.substr(0,3) != "lib")
                                ...
                                //run_command_pacman(("\"C:\\msys64\\ucrt64.exe\" pacman -Ss mingw-w64-ucrt-x86_64-" + i).c_str());
